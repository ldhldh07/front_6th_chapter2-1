...
export const usePoints = () => {
  ...
  const getCartProductTypes = useCallback((cartItems: CartItem[], products: Product[]): ProductTypes => {
    return cartItems.reduce<ProductTypes>((types, cartItem) => {
      ...
      return {
        ...types,
        hasKeyboard: types.hasKeyboard || product.id === PRODUCT_IDS.KEYBOARD,
        hasMouse: types.hasMouse || product.id === PRODUCT_IDS.MOUSE,
        hasMonitorArm: types.hasMonitorArm || product.id === PRODUCT_IDS.MONITOR_ARM
      };
   ...
   * 화요일 보너스 포인트를 계산합니다
   */
  const calculateTuesdayBonus = useCallback((basePoints: number): TuesdayPointsBonus => {
   ...
   * 콤보 보너스 포인트를 계산합니다
   */
  const calculateComboBonuses = useCallback((productTypes: ProductTypes): PointsBonus[] => {
   ...
   * 대량구매 보너스 포인트를 계산합니다
   */
  const calculateBulkBonus = useCallback((itemCount: number): PointsBonus | null => {
  ...
  const calculatePoints = useCallback((
    ...
    const tuesdayDetails = tuesdayBonus.applied ? [tuesdayBonus.description] : [];
    const afterTuesdayPoints = tuesdayBonus.applied ? tuesdayBonus.points : initialPoints;
    ...
    // 최종 계산
    const bonusDetails = [...basePointDetails, ...tuesdayDetails, ...comboDetails, ...bulkDetails];
  ...
  }, [calculateBasePoints, calculateTuesdayBonus, calculateComboBonuses, calculateBulkBonus, getCartProductTypes]);
